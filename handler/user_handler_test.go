...
func TestUserHandler_HandleRetrievalOfLoggedUserSettings(t *testing.T) {
	const (
		method  = "GET"
		target  = "/me/settings"
		routine = "FetchSettings"
	)
	...
	t.Run("success", func(t *testing.T) {
		var (
			...
			serviceResult = &types.Result[transfer.UserSetting]{
				Page:      pagination.Page,
				RPP:       pagination.RPP,
				Payload:   make([]*transfer.UserSetting, 2),
				Retrieved: 2,
			}
			expectedStatusCode   = http.StatusOK
			expectedResponseBody = string(marshal(t, serviceResult))
		...
		withLoggedUser(&request)
		var s = mocks.NewUserServiceMock()
		s.On(routine, userID, &pagination, search, sortExpr).Return(serviceResult, nil)
		var recorder = httptest.NewRecorder()
		NewUserHandler(s).HandleRetrievalOfLoggedUserSettings(recorder, request)
		var response = recorder.Result()
		defer response.Body.Close()
		var responseBody = extractResponseBody(t, response.Body)
		assert.Equal(t, expectedResponseBody, string(responseBody))
	...
	t.Run("got an unexpected service error", func(t *testing.T) {
		...
		withLoggedUser(&request)
		var s = mocks.NewUserServiceMock()
		s.On(routine, mock.Anything, mock.Anything, mock.Anything, mock.Anything).Return(nil, unexpected)
		var recorder = httptest.NewRecorder()
		NewUserHandler(s).HandleRetrievalOfLoggedUserSettings(recorder, request)
		var response = recorder.Result()
		defer response.Body.Close()
